...
PIXELS = 60
#FTRAIN = 'Train_Data_60x60_pixels.pkl.gz'
FTRAIN = 'Train_Data_60x60_pixels_fix.pkl.gz'
FTEST =  'Test_Data_60x60_pixels.pkl.gz'
...
#from dnn import *
#Conv2DDNNLayer = Conv2DDNNLayer
#MaxPool2DDNNLayer = MaxPool2DDNNLayer
...
class AdjustVariable_custom_3(object):
    def __init__(self, name, start=0.03, stop=0.001):
        self.name = name
        self.start, self.stop = start, stop
        self.ls = None
    ...
    def __call__(self, nn, train_history):
        ...
        epoch = train_history[-1]['epoch']
        ls_tmp = np.linspace(0.03, 0.001, 65)
        new_value = np.cast['float32'](ls_tmp[epoch - 1])
        print "Current lr = ", new_value
        getattr(nn, self.name).set_value(new_value)


        
class AdjustVariable_custom(object):
    def __init__(self, name, start=0.03, stop=0.001):
        self.name = name
        self.start, self.stop = start, stop
        self.ls = None
    ...
    def __call__(self, nn, train_history):
        ...
        epoch = train_history[-1]['epoch']
        ls_tmp = np.linspace(0.03, 0.001, 60)
        if epoch <= 60:
            new_value = np.cast['float32'](ls_tmp[epoch - 1])
        else:
            new_value = 0.0001
